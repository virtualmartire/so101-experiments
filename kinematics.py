...
try:
    ...
    def require_package(package_name: str, extra: str | None = None) -> None:
        """Compatibility fallback for LeRobot versions without require_package."""
        try:
            importlib.import_module(package_name)
        except ImportError as exc:
            extra_msg = f" Install LeRobot with the `{extra}` extra." if extra else ""
            raise ImportError(f"Missing optional dependency `{package_name}`.{extra_msg}") from exc
